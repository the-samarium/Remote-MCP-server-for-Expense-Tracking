...
@mcp.tool()
async def return_highest_expense(start_date, end_date):
    '''Return the highest expense within the given date range.'''
    try:
        async with aiosqlite.connect(DB_PATH) as c:  # Changed: added async
            cur = await c.execute(  # Changed: added await
                """
        ...
        return {"status": "error", "message": f"Error returning highest expense: {str(e)}"}

@mcp.tool()
async def expenses_from_category(category, start_date, end_date):
    ...
    try:
        ...
        return {"status": "error", "message": f"Error returning expenses from category: {str(e)}"}
